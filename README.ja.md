...
3. ツールバーのアイコンから「曲を開く」（手元の `.M`/`.m`/`.muc` ファイル、または
   ...
   ができます。ファイルから開く・ドラッグ&ドロップは、ZIP/LZHで固めた書庫
   （`.d88` ディスクイメージを含む書庫も可）にも対応しており、中身を展開して
   曲を選べます（下記のURLから書庫を開く場合と同じ画面です）。
...
6. FMDSP画面のパート行をクリックすると、そのパートだけミュートできます（もう一度
   クリックで解除。ソロ再生機能はありません）。ミュート中の行は元の色のまま暗く
   表示され、鍵盤ハイライトも同じ調子で暗くなります。リズムパートはパート行を
   持たないため、右側のレベルメーターのRHY列をクリックしてミュートします
   （FM1〜SSG3/ADPCMの列はパート行と同じミュート状態を指します。PPZの列は
   ミュート非対応です）。ミュートできる場所（パート行・レベルメーターの列）に
   マウスを重ねるとクリック前に枠で確認できます。パート表示は「通常」「ミュート中
   （半分の明るさ）」「曲が使っていない（4分の1の明るさ、このアプリでMMLを
   コンパイルして再生した場合のみ判定可能。あらかじめコンパイル済みの`.M`/`.m`を
   読み込んだ場合は非対応）」の3段階です。曲を読み込み直すとミュートは全て
   解除されます。マウス/タッチのみ対応で、キーボードからの操作には未対応です。
7. ヘッダーの「言語」ボタンで UI の日本語/英語を切り替えられます（ボタンには
   押すと切り替わる先の言語名が表示されます）。選択はブラウザに記憶され、
   `?lang=ja`/`?lang=en` より優先されます（どちらも無い場合はブラウザの言語設定
   にフォールバックします）。
8. ヘッダーのヘルプアイコンから、スクリーンショット付きの使い方ページ
   （`html/help.html`、日英対応）を開けます。
9. ツールバーの「リンクをコピー」ボタンで、いま編集中のMMLを共有できるURLを
   コピーできます（アップロードは一切発生しません。曲データはURLのフラグメント
   `#s1=...` に載せているだけで、フラグメントはブラウザがサーバーへ送らない
   ため、GitHub PagesのようなサーバーレスのホスティングのままでOKです）。
   ボタンの隣には、いまのリンクの文字数（例 `2344 / 4000`）とゲージが常に
   表示されます。打鍵のたびではなくコンパイル&再生のタイミングで集計し、
   編集中（未反映の変更がある間）は「未集計」と表示され、共有ボタンを押した
   瞬間に必ず再集計されます。**上限の4,000字を超えるとボタンが無効化され、
   何字ぶん超えているかが表示されます**（上限が存在する理由: Xなどが
   4,000字を超えるURLを通常のリンクとして扱わないため。実測の詳細は
   `net/share-link.js` 冒頭のコメント参照）。クリップボードへのコピーに
   失敗した場合だけ、リンクを表示する読み取り専用のテキスト欄が出るので、
   そこから手動でコピーできます。MUCOM88で、曲がディスク固有の外部音色
   バンク（MML自体には書かれておらず、対になるシステムディスクから解決した
   `#voice`）を使っている場合は、共有時にその旨の警告が出ます。受け取った側は
   そのディスクを持たないため、音色が既定のものに変わります。

## できないこと（重要）

現状のプレーヤーには、正直に書いておくべき制約がいくつかあります。

- **`#voice`、および同梱の標準バンク以外を指す `#pcm` で指定される外部ファイルは
  読み込めません。** `#pcm mucompcm.bin`（標準ADPCMバンク）は同梱済みで通常どおり
  鳴りますが、それ以外のファイル名を指す `#voice` は音色が既定のものになります。
  `#pcm` の方は解決できなくても無音にはならず、標準ADPCMバンクがそのまま
  使われるため、ドラム(ADPCM)が本来と異なる音で鳴ります。読み込んだ MML が
  この形で解決できないファイルを参照している場合は、画面上にその旨を表示します
  （音色とドラムが本来と異なる、という注意）。
- **リズムパートは、本物の YM2608 とは異なる代替サンプルで鳴ります。**
  ...
  波形が異なる」と明言している代替品である点をご了承ください(以前は`#voice`/`#pcm`
  と同様に画面上へ注意書きを出していましたが、リズムは「鳴らない」から「鳴るが
  波形が違う」へ実害が下がったため、2026-08-16 以降は画面表示は行いません。
  この制約自体はそのまま残っています)。
- **MUCOM88のコンパイルエラーメッセージは、画面の表示言語設定に関わらず常に
  日本語で出ます。** wasm内に組み込まれた日本語のエラーテーブル由来のため、
  UIを英語にしていてもエラー本文だけは日本語のままです。
- **PPZ8・LFO・ポルタメント等、PMD コンパイラ（自作の MML→バイナリ変換）が
  対応する範囲は v1 の基本コマンドまでです。** 詳細は `docs/pmd-compiler-spec.md`
  参照。
- **MUCOM88のK(ADPCM)パートは、音量コマンドに`v255`(最大値)を指定すると
  逆にほとんど聞こえなくなります。** `v200`程度までは音量が素直に上がりますが、
  最大値の`v255`だけ音量が大きく落ちる不具合があるため、最大値の指定は
  避けてください。
- **右ペイン(FMDSP)の CPU POWER COUNT・VOLUME DOWN・PGM NUMBER は値を
  出せません**（数値の代わりに暗色表示になります）。CPU POWER COUNTは
  ブラウザにプロセス/タブのCPU使用率を取得するAPIが存在しないため恒久的に
  取得不可能です。VOLUME DOWN・PGM NUMBERはwasm exportの欠落ではなく、
  上流のFMDSP描画実装自体にこれらの値を描くコードが無く、参照できる共通の
  ドライバインタフェースにも元データが存在しません。一方 FRAMES PER SECOND
  は実装済みです（ドライバのデータではなく、ホスト側の描画ループの実測
  頻度を数えているだけのため）。詳細は `docs/right-pane-data.md` §8 参照。
- **レベルメーターの右側8本(PPZ8 1-8)は常に暗く「未使用」表示のままで、
  決して光りません。** PPZ8はPMDの8chPCM音源です。MUCOM88にはPPZ8という
  概念自体が存在せず、PMD側はドライバコード上PPZ8ミキサーの配線こそ
  ありますが、本Web版はUIからPPZ8のサンプルバンク(.PPC/.PVI)を一切
  読み込めないため、どちらのエンジンでもこれらのチャンネルは鳴らしようが
  ありません。同じ理由でミュートクリック・ホバー枠の対象からも除外して
  います。詳細は `fmdsp/channel-mask.js` の `unusedColumnsFromChannels()`
  参照。
- **スマホは現時点では画面が最適化されておらず、対応を予定しています。** タブレットは現時点では動作確認・最適化を行っていません。
- **ディスク固有の外部音色バンクに依存するMUCOM88の曲を共有すると、受け取った側では
  音が変わります。** 共有リンク（`?driver=` + `#s1=...`）はMMLテキストしか運ばず、
  対になるシステムディスクの `voice.dat` は含まれないため、リンクを開いた相手は
  既定音色で聞くことになります。該当する場合は共有時にUIが警告します（上記の
  使い方参照）。
...
使い方ページ（`html/help.html`）のスクリーンショットは `tools/gen_help_shots.mjs`
で再現可能に生成しています（macOS専用。手元のGoogle Chromeをheadlessで起動し
DevTools Protocol経由で操作する。npm依存の追加なし）。`tools/build_dist.sh` の
あと、`dist/` が最新の状態で実行してください。

```bash
node tools/gen_help_shots.mjs
```

`dist/` をポート8790で配信し、`html/help/<name>.<lang>.png` を書き出します。
撮影対象の画面が実際に変わらない限り、再実行の必要はありません。
