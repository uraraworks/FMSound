...
## 症状⑧(2026-08-15): MUCOM88の同梱ループ曲が約6.5秒で止まる(自動終了検出の誤発火)

`updateTransportButtonUI()`自体ではなく、その隣で動いていた「曲が終わったら
自動的に`stopPlayback()`を呼ぶ」機能(課題B、2026-08-14実装)の話だが、この
機能もPlayボタンの状態(`hasPlayback`)を書き換えるため、状態遷移の一つとして
ここに記録する。

PMD(`fmdriver_work.playing`という専用フラグを使う)では問題無かったが、
MUCOM88側は代替として PCHDATA.flag bit0(LOOPEND FLAG)+ 音程コード(`code`)の
安定判定を組み合わせた独自ロジックを使っていた。当時の検証
(`tools/verify_song_end_detection.mjs`)は単純な単音階の合成MML
(`cdefgab>c<`の繰り返し、休符・和音・複数パート無し)だけで確認しており、
実物の同梱サンプル(`tools/sample_fur_elise_mucom.mml`、2パート・休符あり)
では確認していなかった。

実機報告(ループ曲が約6.5秒=ちょうど1周の長さで止まる)を受けて実物のMMLで
再測定したところ、**bit0は「authored MMLデータの末尾に到達した」時点で一度
立つと、ループが継続していても二度と下がらない**ことが判明した(ループ点と
曲の終了を、そもそも区別する情報を持っていなかった)。休符や同音連打で
「codeが3ポーリング以上変化しない」瞬間はごく普通に起きるため、ループの
1周目が終わるたびに誤発火していた(実測: `tools/verify_song_end_detection.mjs`
のE、260ポーリング中46回誤ってtrueになった)。

対処: **MUCOM88側の自動終了検出機能そのものを撤去した**(`html/mucom-app.js`
の`checkMucomSongEnded`/`resetMucomEndState`と、rAFループでの呼び出しを削除)。
「ループする曲が途中で止まる」方が「ループしない曲が終了後も鳴り続ける」より
明確に害が大きいと判断し、区別できない機能は落とす方を選んだ。手動Stopボタンは
影響を受けない。PMD側は専用フラグがあるため撤去していない(左右非対称になるが、
利用可能な一次情報がドライバごとに違う以上、やむを得ない)。
