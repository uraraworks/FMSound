class PmdStreamProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.chunks = [];
    this.chunkOffset = 0;
    ...
    this.port.onmessage = (event) => {
      ...
      if (message.type === 'start') {
        ...
        this.chunks.push(new Int16Array(message.samples));
  ...
  process(_inputs, outputs) {
    ...
    let outputOffset = 0;
    while (outputOffset < left.length && this.chunks.length) {
    ...
    if (++this.quantumCount % 10 === 0) {
      this.port.postMessage({
        type: 'playback', playFrame: this.playFrame,
        contextTime: currentTime + left.length / sampleRate
      });
    }
    if ((this.quantumCount & 15) === 0) {
      this.port.postMessage({
        type: 'stats', queuedFrames: this.queuedFrames, underflowFrames: this.underflowFrames
      });
    }
    return true;
  }
...
registerProcessor('pmd-stream-processor', PmdStreamProcessor);
