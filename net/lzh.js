...
// PC98/WebNP2/src/api/lzh.ts の移植(型注釈を除いただけで、ロジックは無改変)。
...
class BitReader {
  ...
  constructor(buf, start, end) {
    ...
    this.pos = start;
    this.end = end;
    this.bitBuf = 0; // 32bit windowの上位ビットから詰めていく
    this.bitCount = 0;
  }
  ...
  /** 上位nビット(n<=16)を消費せず覗き見る。 @param {number} n */
  peekBits(n) {
    if (n === 0) return 0;
    this.fill();
  ...
  /** nビット分読み進める(値は返さない)。 @param {number} n */
  skipBits(n) {
    if (n === 0) return;
...
/** @param {{ sym?: number, left?: any, right?: any }} n */
function isLeaf(n) {
  return 'sym' in n;
}

/** 符号長配列(DEFLATE等と同じ正準ハフマン割当規則)からデコード用の二分木を構築する。 @param {number[]} lengths */
function buildCanonicalTree(lengths) {
  let maxBits = 0;
...
/** @param {BitReader} reader @param {any} tree */
 ...
 * @param {BitReader} reader @param {number} nn @param {number} nbit @param {number} iSpecial
 */
function readPtLen(reader, nn, nbit, iSpecial) {
  const n = reader.getBits(nbit);
  if (n === 0) {
    const c = reader.getBits(nbit);
    return { sym: c };
  }
  const lengths = new Array(nn).fill(0);
  let i = 0;
  while (i < n) {
...
/** c木(文字/長さ木、NC要素)の符号長を、直前に読んだpt木経由で読み取る。 @param {BitReader} reader @param {any} ptTree */
function readCLen(reader, ptTree) {
  const n = reader.getBits(CBIT);
  if (n === 0) {
    const c = reader.getBits(CBIT);
    return { sym: c };
  }
  const lengths = new Array(NC).fill(0);
  let i = 0;
  while (i < n) {
...
/** @param {{ blockRemaining: number, cTree: any, pTree: any, np: number, pBit: number, reader: BitReader }} state */
function decodeChar(state) {
  ...
  return decodeSymbol(state.reader, state.cTree);
}

/** @param {{ pTree: any, reader: BitReader }} state */
function decodePosition(state) {
  let j = decodeSymbol(state.reader, state.pTree);
  if (j > 1) {
...
/** lh5/6/7 共通のLZSS+ハフマン展開。 @param {Uint8Array} compressed @param {number} originalSize @param {number} dicBit @param {number} pBit */
function decodeLzss(compressed, originalSize, dicBit, pBit) {
 ...
 * @param {Uint8Array} bytes @param {number} start @param {string} baseName
 */
function parseExtendedHeaders(bytes, start, baseName) {
  let pos = start;
  let name = baseName;
  let dirPrefix = '';
 ...
 */
export function extractLzh(bytes) {
  const entries = [];
  let pos = 0;
