...
右半分(FMDSP_RIGHT_MODE_DEFAULT、ORIGINAL相当)向けに追加した抽出対象:
  ...
  s_playing      : "PLAYING"ラベルのビットマップ(fmdsp_sprites.h:621-629,
                   PLAYING_W x PLAYING_H)。曲名バー(FILEBARの本体)実装
                   (2026-08-15)で追加。
"""
import re
...
# 2026-08-15: 実際の出力先は単一化されたトップレベル fmdsp/ (html/mucomweb/pmdweb は
# tools/build_dist.sh 等がここからコピーする)。旧 pmdweb/html/fmdsp/ 構成の名残の
# パスになっていたのを修正した(スクリプトのロジック自体は既存の fmdsp/sprites.js と
# 完全一致することをdiffで確認済み。パス以外の変更はs_playing抽出の追加のみ)。
DST = REPO_ROOT / "fmdsp/sprites.js"
...
# fmdsp_sprites.h:47-48
PLAYING_W, PLAYING_H = 72, 9
...
def main() -> int:
    ...
    s_playing = extract_array(text, "s_playing", "PLAYING_W*PLAYING_H")
    if len(s_playing) != PLAYING_W * PLAYING_H:
        raise SystemExit("error: s_playing size mismatch")
    ...
    lines.append(f"export const PLAYING_W = {PLAYING_W};")
    lines.append(f"export const PLAYING_H = {PLAYING_H};")
    ...
    lines.append(
        "// \"PLAYING\"ラベルのビットマップ(fmdsp-pacc.c:1843-1844で"
        " PLAYING_X=0,PLAYING_Y=324に描画)。曲名バー(FILEBAR)実装(2026-08-15)で追加。"
    )
    lines.append(f"export const S_PLAYING = {js_array(s_playing)};")
    lines.append("")
