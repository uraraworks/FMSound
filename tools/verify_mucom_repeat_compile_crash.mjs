#!/usr/bin/env node
// 実機報告「新規作成/ライブラリ読込/共有リンク貼付を繰り返すとwasmがmemory access
// out of boundsで落ちる」の再現ハーネス(2026-08-17調査)。
//
// 【2026-08-17追記・対症療法適用済み】mucomweb/patches/0005-cmucom-gettextline-bounds.patch
// で CMucom::GetTextLine()/strpick_spc() へ境界チェックを追加した後は、このハーネスを
// 2000回まで回してもクラッシュしない(旧ビルドでは以前の記述どおり440回目に確実に
// クラッシュすることを陽性対照として再確認済み)。ただし「なぜ440回目という特定の
// 回数で3種ローテーションがstrpick_spcの脆弱点を踏むのか」自体は未解明のまま。
// 詳細はdocs/mucom-repeat-compile-crash.md参照。根本原因を直したと主張するものでは
// ない(対症療法)。
//
// 【結論(このラウンドで判明したこと)】
